...
>     float combinedPadding = (axis ==  ? padding.horizontal :
padding.vertical);
...
>             totalPreferred = Mathf.Max(preferred + combinedPadding,
totalPreferred);
...
>         float innerSize = size - (axis ==  ? padding.horizontal :
padding.vertical);
...
>             float requiredSpace = Mathf.Clamp(innerSize, min, flexible >  ?
size : preferred);
...
>         (GetTotalFlexibleSize(axis) ==  && GetTotalPreferredSize(axis) <
size)

>

>             pos = GetStartOffset(axis, GetTotalPreferredSize(axis) - (axis
...
>                 itemFlexibleMultiplier = (size -
GetTotalPreferredSize(axis)) / GetTotalFlexibleSize(axis);
...
> Button1:10 + 0.3/0.4 _(100 - 20) = 70_
...
> public static float GetLayoutProperty(RectTransform rect,
System.Func<ILayoutElement, float> property, float defaultValue,
...
>         (layoutComp  Behaviour && (!(layoutComp  Behaviour).enabled ||
!(layoutComp  Behaviour).isActiveAndEnabled))
...
>         // If this layout components has lower priority than a previously
used, ignore it.
...
>         // If this layout property is set to a negative value, it means it
should be ignored.
...
>         // If this layout component has higher priority than all previous
ones,
...
>         // If the layout component has the same priority as a previously
used,
